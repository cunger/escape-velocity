...
* Die Ableitung ist der Differentialquotient, also der Proportionalitätsfaktor zwischen infinitesimalen änderungen des Eingabewertes und daraus resultierenden infinitesimalen änderungen des Funktionswertes. An einer Stelle {{< katex "x_0" >}}:

  {{< katex display="f'(x_0) = \text{lim}_{x\to x_0}\frac{f(x)-f(x_0)}{x-x_0}" >}}

  Das Problem bei Funktionen {{< katex "f:\mathbb{R}^n\to\mathbb{R}^m" >}} ist, dass {{< katex "x-x_0\in \mathbb{R}^n" >}} und {{< katex "f(x)-f(x_0)\in \mathbb{R}^m" >}}, der Quotient beider ist also gar nicht vernünftig definiert. Deswegen weicht man da auf den anderen Ableitungsbegriff aus.

* Die Ableitung ist die Steigung der linearen Funktion, die die änderung der betrachteten Funktion in dem gegebenen Punkt lokal am besten approximiert. Eine Funktion ist also differenzierbar in einem Punkt, wenn sie in dem Punkt "annähernd linear" ist.

Differential

Richtungsableitung: das lokale Verhalten der Komponentenfunktion bei änderung dieser Komponente.

Gradient

Die partielle Ableitung einer Funktion mehrerer Veränderlicher ist nichts anderes als die Ableitung der Funktion, die man erhält, wenn man alle Veränderlichen bis auf eine festhält.
